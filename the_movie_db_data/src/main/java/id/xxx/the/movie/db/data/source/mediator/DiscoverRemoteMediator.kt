...
import id.xxx.the.movie.db.data.source.local.base.IResultEntity
...
abstract class DiscoverRemoteMediator<Key : Any, LocalResult : IResultEntity, RemoteResult : IResult> :
    ...
    final override suspend fun load(
        ...
        return try {
            val results: List<PageResponse<RemoteResult>?> = withContext(Dispatchers.IO) {
                ...
                List(pageSize / MIN_PAGE_INDEX_SIZE) {
                    async {
                        ...
                        when (val result = fetch(currentPage)) {
                            ...
                            is ApiResponse.Error -> throw Throwable(result.error)
                        }
                    }
